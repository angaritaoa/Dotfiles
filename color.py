...
# (c) 2012-2020 Mantas Mikulėnas <grawity@gmail.com>
...
class State(dict):
    ...
    def load(self, data, recurse=True):
        ...
        for prop in properties:
            self[prop] = data.get(prop, properties[prop][0])
    ...
    def load_persistent(self):
        try:
            ...
            print("State file could not be loaded (%s), moving aside." % e,
                  file=sys.stderr)
            os.rename(state_path, state_path + "~")
    ...
    def getcolor(self, iso=False):
        if iso and self["mode"] == "iso":
            return self["color"] + self["bright"]*60
        elif self["mode"] == "sys":
            return self["color"] + self["bright"]*8
        elif self["mode"] == "rgb":
...
def main(state):
    ...
    if state["mode"] in {"rgb", "rgb888"}:
        ...
        print(indent, "color +/- (q/e), bright 1/3 (z/c), reset 0")
    elif state["mode"] == "gray":
        print(indent, "level +/- (q/e), reset 0")
    print()

    if state["mode"] == "rgb":
        bar("red",   state, "red",   4, "7/9 or q/e")
        bar("green", state, "green", 4, "4/6 or a/d")
        bar("blue",  state, "blue",  4, "1/3 or z/c")
    elif state["mode"] in {"sys", "iso"}:
        bar("color", state, "color", 4, "+/- or q/e")
        bar("bright",state, "bright",16, "1/3 or z/c")
    elif state["mode"] == "gray":
    ...
    if k == "Q":        return False
        ...
        if None: pass
        elif k in "74qa-":      state.decr("color")
        elif k in "852wsx0":    state.reset("color", "bright", "flags")
        elif k in "96ed+":      state.incr("color")
        elif k in "1z":         state.decr("bright")
        elif k in "3c":         state.incr("bright")
        elif k in "/":          state.decr_carry("color", "bright")
        elif k in "*":          state.incr_carry("color", "bright")
    elif state["mode"] == "iso":
        if None: pass
        elif k in "74qa-":      state.decr("color")
